...
def find_heating_output(temp_price_df: pd.DataFrame,
    ...
    if (values["HeaterOutput"] is None or values["CoolingOutput"] is None
            or values["_T"] is None):
        raise SolverError(
            f"Solver reported {problem.status!r} but returned no variable "
            "values; no schedule is available."
        )
